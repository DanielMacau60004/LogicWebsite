...
import logic.algorithm.backups.backup6.implementation.Edge;
import logic.algorithm.implementation.Solution;
import logic.algorithm.implementation.transition.TransitionEdge;
import logic.algorithm.implementation.transition.TransitionGraph;
import logic.algorithm.implementation.transition.TransitionNode;
...
public class StateGraph {
    ...
    void trim(Queue<StateNode> explore, Map<StateNode, Set<StateEdge>> inverted) {
        List<StateNode> leaves = new ArrayList<>(explore);
    ...
    public Solution findSolution() {
        ...
        return findSolutionAux(getInitState());
    }
    ...
    public Solution findSolution(Exp exp, Set<Exp> hypotheses) {
        ...
        return findSolutionAux(new StateNode(exp, premisses, hypotheses));
    }

    public Solution findSolution(Exp exp, Set<Exp> hypotheses, Set<Exp> notFree) {
        if (!isSolvable())
            return null;

        return findSolutionAux(new StateNode(exp, premisses, hypotheses, notFree));
    }

    private Solution findSolutionAux(StateNode initState) {
        Set<StateEdge> edges = graph.get(initState);

        if (edges == null) {
            return new Solution(initState, null);
        }

        StateEdge edge = edges.stream().findFirst().get();
        Solution solution = new Solution(initState, edge.getRule());

        for (StateNode node : edge.getTransitions()) {
            Solution newSolution = findSolutionAux(node);
