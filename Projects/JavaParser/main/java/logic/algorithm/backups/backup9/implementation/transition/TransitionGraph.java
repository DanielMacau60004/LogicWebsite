package logic.algorithm.backups.backup9.implementation.transition;
...
public class TransitionGraph {
    ...
    public TransitionGraph(Exp conclusion, Set<Exp> premisses) {
        ...
        //TODO MAYBE NOT THE BEST APPROACH!
    ...
    private void existentialIRule(ASTExistential exi) {
        ...
        terms.forEach(t -> {
            ...
            if (!FOLFreeInterpreter.isFree(psi, xVar)
                    || !FOLReplaceVariables.replace(psiXT, tTerm, xVar).equals(psi))
                return;
    ...
    private void universalERule(ASTUniversal uni) {
        ...
        terms.forEach(t -> {
            ...
            if (FOLFreeInterpreter.isFree(psiXT, tTerm))
                addEdge(psiXT, new TransitionEdge(ERule.ELIM_UNIVERSAL)
                        .addTransition(uni), true);
        });
    }

    //TODO panic could not occur free in the free hypotheses
    private void universalIRule(Exp exp, ASTUniversal uni) {
        Exp psi = ExpUtils.removeParenthesis(uni.exp);
        ASTVariable xVar = (ASTVariable) uni.variable;
