...
public class StateGraph {
    ...
    private void build(int heightLimit, int nodesLimit) {
        ...
        explore.add(new StateNode(transitionGraph.getConclusion(), transitionGraph.premisses));
            ...
            if (state.isClosed()) closed.add(state);
                ...
                for (TransitionEdge edge : transitionGraph.getEdges(state)) {
                    ...
                    for (Transition transition : edge.getTransitions()) {
                        StateNode newState = state.transit(transition.to, transition.produces, transition.requiresToBeClosed);
    ...
    public boolean isSolvable() {
        StateNode node = new StateNode(transitionGraph.getConclusion(), transitionGraph.premisses);
        return graph.containsKey(node);
    }

    private void trim(Map<StateNode, Set<StateEdge>> inverted) {
                ...
                for (StateEdge prev : inverted.get(state)) {
                    for (StateNode to : prev.getTransitions()) {
                        if (graph.get(to) != null && graph.get(to).stream().anyMatch(StateEdge::isClosed)) {
                            explore.add(to);
                            StateEdge e = graph.get(to).stream().filter(StateEdge::isClosed).findFirst().get();
                            to.updateClose(e.getTransitions());
                            //graph.get(to).removeIf(s -> !s.isClosed());
                        }
                    }
                }
            }
    ...
    public List<Solution> findSolutions(int limit, StateNode initState, Set<ERule> forbiddenRules) {
        ...
        StateNode defaultState = new StateNode(transitionGraph.getConclusion(), transitionGraph.premisses);
        if(initState != null) {
            initState.addHypotheses(transitionGraph.premisses);
            explore.add(new Solution(initState));
        } else explore.add(new Solution(defaultState));
