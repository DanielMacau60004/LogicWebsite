package algorithm.backup1.implementation;
    ...
    public List<Solution> solve(Exp exp, int solutionSize, int solutionsLimit) {
        ...
        while (!queue.isEmpty()) {
            ...
            for (Edge edge : graph.get(current)) {
                ...
                if(current instanceof ASTAnd and) {
                    Solution newSolution = solution.clone();
                    newSolution.addPath(removeParenthesis(and.right));

                    Solution newSolutionLeft = solution.clone();
                    newSolutionLeft.addPath(removeParenthesis(and.left));
                    newSolution.addNextProof(newSolutionLeft);
